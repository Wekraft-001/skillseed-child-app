...
import { ArrowLeft, Star, Code, Bot, Map, Trophy, Brain, Rocket, Users, MessageSquare, Lightbulb } from 'lucide-react';
import { PageMetadata } from '@/components/PageMetadata';

// Define the Child type to match the one in MainContent
type Child = {
  id: string;
  name: string;
  age: number;
    ...
    <>
      ...
      <div className="bg-[#F5F7FA] min-h-[800px]">
        ...
        <div className="p-6 flex items-center gap-3">
          <Link to="/" className="text-[#1A73E8] flex items-center gap-2 cursor-pointer">
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm">
            <div className="flex items-center gap-6">
              <div className="relative">
                <img src={child.avatar} className="w-24 h-24 rounded-full border-4 border-[#FFC107]" alt={`${child.name}'s Avatar`} />
              ...
              <div>
                ...
                <p className="text-gray-500">Age {child.age} • Grade {child.age - 6}</p>
                <div className="flex gap-3 mt-3">
                  {child.interests.map((interest, index) => (
                    <span key={index} className="bg-blue-100 text-[#1A73E8] px-3 py-1 rounded-full text-sm">{interest}</span>
                  ))}
                </div>
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm">
            ...
            <div className="space-y-4">
              <div className="flex items-start gap-3">
                ...
                <p><span className="font-medium">Interests:</span> {child.interests.join(', ')}</p>
              </div>
              <div className="flex items-start gap-3">
                <Trophy className="w-5 h-5 text-[#FFC107] mt-0.5 flex-shrink-0" />
                <p><span className="font-medium">Skills:</span> {child.skills.join(', ')}</p>
              </div>
              <div className="flex items-start gap-3">
                <Lightbulb className="w-5 h-5 text-[#FFC107] mt-0.5 flex-shrink-0" />
                <p><span className="font-medium">Dream Job:</span> {child.dreamJob}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="px-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-8 space-y-6">
            {/* Progress Overview */}
            <div id="progress-overview" className="bg-white rounded-xl p-6 shadow-sm w-full">
              <h2 className="text-lg font-semibold mb-6">Learning Progress</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="text-center p-4 bg-blue-50 rounded-xl">
                  <div className="text-[#1A73E8] text-2xl font-bold">{child.completedActivities.length}</div>
                  <p className="text-sm text-gray-600">Completed Activities</p>
                </div>
                <div className="text-center p-4 bg-yellow-50 rounded-xl">
                  <div className="text-[#FFC107] text-2xl font-bold">{child.inProgressActivities.length}</div>
                  <p className="text-sm text-gray-600">In Progress Activities</p>
                </div>
                <div className="text-center p-4 bg-green-50 rounded-xl">
                  <div className="text-green-500 text-2xl font-bold">{child.upcomingSessions.length}</div>
            ...
            {/* Recent Achievements */}
            <div id="recent-achievements" className="bg-white rounded-xl p-6 shadow-sm">
              <h2 className="text-lg font-semibold mb-6">Recent Achievements</h2>
              <div className="space-y-4">
                {child.completedActivities.map((activity, index) => (
                  <div key={index} className="flex items-center gap-4 p-4 bg-gradient-to-r from-blue-50 to-white rounded-xl">
                    <div className="bg-[#1A73E8] p-3 rounded-full text-white">
                      <Code className="w-5 h-5" />
                    </div>
                    <div>
                      <h3 className="font-semibold">{activity}</h3>
                      <p className="text-sm text-gray-500">Achieved 95% score in final assessment</p>
                      <p className="text-xs text-[#1A73E8] mt-1">May 12, 2025</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            {/* Skill Development */}
            <div id="skill-development" className="bg-white rounded-xl p-6 shadow-sm">
          ...
          <div className="lg:col-span-4 space-y-6">
            {/* Career Interests */}
            <div id="career-interests" className="bg-white rounded-xl p-6 shadow-sm w-full">
            ...
            <div id="scheduled-excursions" className="mb-8">
              <div className="bg-white rounded-xl p-6 shadow-sm">
                <h2 className="text-xl font-semibold mb-4">Scheduled Excursions</h2>
                <div className="space-y-4">
                  {child.scheduledExcursions.length > 0 ? (
                    child.scheduledExcursions.map((excursion, index) => (
                      <div key={index} className="border border-gray-200 p-3 rounded-lg">
                        ...
                        <p className="text-sm text-gray-500">Date: {excursion.date}</p>
                        <p className="text-sm text-gray-500">Location: {excursion.location}</p>
            ...
            {/* Upcoming Activities */}
            <div id="upcoming-activities" className="bg-white rounded-xl p-6 shadow-sm w-full">
              ...
              <div className="space-y-4">
                {child.upcomingSessions.length > 0 ? (
                  child.upcomingSessions.map((session, index) => (
                    <div key={index} className="border-l-4 border-[#1A73E8] pl-3">
                      <p className="font-medium">{session.mentor}</p>
                      <p className="text-sm text-gray-500">Time: {session.time}</p>
                    </div>
                  ))
                ) : (
              ...
              <div className="bg-white rounded-xl p-6 shadow-sm">
                ...
                <div className="space-y-4">
                  ...
                  {child.inProgressActivities.map((activity, index) => (
                    <div key={`inprogress-${index}`} className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
